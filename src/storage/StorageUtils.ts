...
const deepSeekApiUrlKey = keyPrefix + 'deepSeekApiUrlKey';
const openAIApiUrlKey = keyPrefix + 'openAIApiUrlKey';
...
let currentDeepSeekApiUrl: string | undefined;
let currentOpenAIApiUrl: string | undefined;
...
export function getDeepSeekApiUrl(): string {
  if (currentDeepSeekApiUrl) {
    return currentDeepSeekApiUrl;
  } else {
    currentDeepSeekApiUrl = storage.getString(deepSeekApiUrlKey) ?? 'https://api.deepseek.com';
    return currentDeepSeekApiUrl;
  }
}
...
export function getOpenAIApiUrl(): string {
  if (currentOpenAIApiUrl) {
    return currentOpenAIApiUrl;
  } else {
    currentOpenAIApiUrl = storage.getString(openAIApiUrlKey) ?? 'https://api.openai.com';
    return currentOpenAIApiUrl;
  }
}
...
export function getHapticEnabled() {
  return storage.getBoolean(hapticEnabledKey) ?? false;
}
...
export function saveDeepSeekApiUrl(apiUrl: string) {
  currentDeepSeekApiUrl = apiUrl;
  storage.set(deepSeekApiUrlKey, apiUrl);
}
...
export function saveOpenAIApiUrl(apiUrl: string) {
  currentOpenAIApiUrl = apiUrl;
  storage.set(openAIApiUrlKey, apiUrl);
}
